      ...
      <div class="auth-card">
        ...
        <form (ngSubmit)="onSubmit()">
          ...
          <button type="submit" class="btn-primary" [disabled]="isLoading">
            {{ isLoading ? ('AUTH.SIGNING_IN' | translate) : ('AUTH.LOGIN' | translate) }}
          </button>
        </form>
        <p class="auth-link">{{ 'AUTH.NO_ACCOUNT' | translate }} <a routerLink="/auth/register">{{ 'AUTH.REGISTER' | translate }}</a></p>
      </div>
    </div>
  `,
  styles: [`
